...
function renderStepIndicator(){
  const steps = ["Profil","Drum","Input THG","Pilih Warna","Laporan","Riwayat"];
...
// STEP 6: RIWAYAT SEMUA DRUM
  ...
  const cardsHtml = drums.map(nama => {
    const list = allForDay.filter(d => d.drum === nama); // cocokkan langsung "A", "B", ...
    const latest = list[0]; // karena laporan disimpan pakai unshift
...
function openDetailDrum(nama) {
  ...
  // ambil data untuk tanggal aktif, dan filter berdasarkan drum
  const data = getLaporanHari(currentDateKey).filter(d => d.drum === nama);

  // render semua kolom: No, Waktu, Nama, Drum, Suhu, Kelembapan, Gas, Status
  body.innerHTML = data.length > 0
    ? data.map((d, i) => `
      ...
      `).join("")
    : `<tr><td colspan="8" class="p-4 text-gray-500">Belum ada data</td></tr>`;
...
function closeDetailDrum(){ closeModal("modalDetailDrum"); }
function loadHistoryAll(dateKey){
